...
async function processTweet (tweet) {
  return new Promise((resolve, reject) => {
    ...
    */
    ...
    return twitterClient.post(`statuses/destroy/${tweet.tweet_id}`, async (err, returnedTweet, response) => {
      if (err) {
        ...
        switch (errCode) {
          ...
          default:
            throw new Error(`Unhandled Twitter API error response code ${errCode}`)
        ...
        if (response.statusCode === 200) {
          ...
          throw new Error(`Unhandled Twitter API response status code ${response.statusCode}`)
        }
      }
    })
  })
...
async function recursivelyProcessTweets (tweetsArray, index) {
  processTweet(tweetsArray[index])
    .then(deletedTweet => {
      console.log('================================================================================')
      console.log(`Processing tweet ${index}/${tweetsArray.length} in your archive`)
